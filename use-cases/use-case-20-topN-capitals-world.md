...
### Goal in context
Display the top N populated capital cities in the world ordered from largest to smallest so that users can identify and compare the most populated capital cities globally.
### Scope
...
### Success Condition
- The system displays the top N capital cities ordered by population (largest to smallest).
...
- Research Analyst
### Trigger
The user selects “Top N Capital Cities Report” and specifies the number of capital cities (N).
## MAIN SUCCESS SCENARIO
1. User launches the application.
2. User selects “Top N Capital Cities Report.”
3. System prompts the user to enter the number of capital cities (N).
4. User provides a valid positive integer (e.g., 10).
5. System connects to the world database.
6. System executes the SQL query to retrieve the top N capital cities ordered by population.
7. System formats the retrieved data into a readable table or report.
8. System displays the top N capital cities in the world.
9. User reviews and confirms the output.
## EXTENSION
3. **User does not enter a value for N**:
    1. System displays an error and prompts again.
4. **Invalid input for N (e.g., negative or non-numeric)**:
